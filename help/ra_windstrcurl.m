function [curlZ]=ra_windstrcurl(lat,lon,u,v,opt)
...
% 
% INPUTS:
% lat = Latitude vector [deg.] 单调递增
% lon = Longitude Vector [deg.] 单调递增
% u = Zonal wind component [m/s], must be 2D (lat * lon)
% v = Meridional wind component [m/s], must be 2D (lat * lon)
% if opt = 0, u v are recognized as wind; if opt = other number, u v are
% recognized as wind stress.
%
% OUTPUT: 
...
% 
% REFERENCE:
% A.E. Gill, 1982, 揂tmosphere-Ocean Dynamics?, Academy Press, Vol. 30.
% W. G. Large & S. Pond., 1981,揙pen Ocean Measurements in Moderate to Strong Winds?, 
% J. Physical Oceanography, Vol. 11, pp. 324 - 336.
% K.E. Trenberth, W.G. Large & J.G. Olson, 1990, 揟he Mean Annual Cycle in Global Ocean 
