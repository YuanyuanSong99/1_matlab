...
%   a normal axis.
clc,clear,close all;
st=load('d:/R2014b/bin/2sst.mat'); % input sss series
st1=st.sst;
...
p_s=st1(120:180,74:94,:);%暖池区
ps_t=nanmean(nanmean(p_s,1),2);%时间序列
ps_t1=[1:1:804];
ps_t1(:)=ps_t(1,1,:)-273.15;
plot(ps_t1);
%最小二乘法拟合
Tm=ps_t1; 
A=[1:1:804];
...
%10年滑动平均
%load('f:/graduation/data/EN4/p_sst10.mat');%data_array_filter
...
xlabel('Time (year)')
ylabel('Period (years)')
title('b) SST Wavelet Power Spectrum')
set(gca,'XLim',xlim(:))
...
saveas(gcf,'f:/graduation/data/EN4/西赤道太平洋暖池sst去趋势后小波分析.png')
% end of code
