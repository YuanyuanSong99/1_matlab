...
Tsubraw = permute(sum(cat(3,Temp(:,lats,1,:),Temp(:,lats,2:27,:).*(permute(dweit,[3 2 1]))),3,'omitmissing')/700,[1 2 4 3]); 
% map plot
map = mean(Tsubraw(:,:,42:71),3,'omitmissing');
close all;
ftsz = 12; ticks = 10;
Fig = figure('position',[10 50 550 400]);
ax = axes('Position',[0.005 0.07 0.9 0.87],'fontsize',ftsz,'box','on');
contourfS(map,[-ticks*5:1:ticks*5],ticks,lonData,latData(lats),12)
...
set_colorbar([0.83 0.08 0.03 0.88],'^oC',5,ftsz,[-ticks:ticks/5:ticks],[-ticks:ticks/5:ticks])
% print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240516_SO_warming/Climo_Tem_',depthstr,'m.png'],'-dpng','-r300')
...
set(gca,'YTickLabel',[2000:-400:0],'FontSize',14);
ylabel('Depth (m)')
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240516_SO_warming/meridionalmean_',latstr,'_trend_',yrstr,'_150E.png'],'-dpng','-r300')
...
map = nanmean(OHCw(:,:,yrs2-1939:yrs2-1939+yrspan-1),3)-nanmean(OHCw(:,:,yrs1-1939:yrs1-1939+yrspan-1),3);
max(map,[],'all')
min(map,[],'all')
% SO asymmetry
close all;
...
% print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240516_SO_warming/Delta_OHC_W_raw',depthstr,'_',num2str(yrs1),'_',num2str(yrs2),'_',num2str(yrspan),'yrs.2.png'],'-dpng','-r300')
%% OHC trend
startyr = 1960;
endyr = 2020;
...
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240516_SO_warming/OHC',depthstr,'_trend_',num2str(startyr),'_',num2str(endyr),'.png'],'-dpng','-r300')
%% Global plot
close all;
ftsz = 12; ticks = 15;
...
Fig = figure('position',[10 50 550 400]);
ax = axes('Position',[0.005 0.07 0.9 0.87],'fontsize',ftsz,'box','on');
contourfS(map,[-ticks*5:ticks/10:ticks*5],ticks,lonDataE,latDataE,ftsz)
m_line([0:1:360],-35,'linewidth',2.5,'color',[.4 .4 .4]); 
m_line([0:1:360],-55,'linewidth',2.5,'color',[.4 .4 .4]); 
m_line(-70,[-55:1:-35],'linewidth',2.5,'color',[.4 .4 .4]); 
m_line(150,[-55:1:-35],'linewidth',2.5,'color',[.4 .4 .4]); 
set_colorbar([0.83 0.08 0.03 0.88],[units,' decade^-^1'],4.5,ftsz,[-ticks:ticks/5:ticks],[-ticks:ticks/5:ticks])
% testdots0(h0,[.4 .4 .4],lonDataE,latDataE);
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240516_SO_warming/',name,'_trend_',num2str(startyr),'_',num2str(endyr),'_3.png'],'-dpng','-r300')
...
%% nhflx
close all
Fig = plotzm2(trd1,trd2,cli1, cli2,'W',...
    'W',-.21,.28,.07,latDataE,-33,44,11)
% 手动保存 NHFLX_zonalm_trend_1960-2020
%% curlz
close all
Fig = plotzm2(trd1*10^9,trd2*10^9,cli1*10^7, cli2*10^7,'10^-^9 Pa m^-^1',...
    '10^-^7 Pa m^-^1',-1.2,0.8,0.4,latDataE,-3.6,2.4,1.2)
...
Fig = plotzm2(trd1*10^4,trd2*10^4,cli1*10^2, cli2*10^2,'10^-^4 Pa',...
    '10^-^2 Pa',-4,10,2,latDataE,-12,30,6)
% 手动保存 UWST_zonalm_trend_1960-2020
% Fig = plotzm2(trd1*10^4,trd2*10^4,'10^-^3 Pa decade^-^1',-4,8,2,latDataE)
...
ylabel(['Trend  (',unit1,'/yr)'],'FontSize',14);
plot(latDataE(121:161),zeros(1,41),'k','LineWidth',1)
set(gca,'YLim',[ymin1,ymax1],'YTick',[ymin1:yint1:ymax1],'YColor','k');
set(gca,'XGrid','on','YGrid','on');
...
set(gca,'YLim',[ymin2,ymax2],'YTick',[ymin2:yint2:ymax2],'YColor','k');
ylabel(['Climo (',unit2,')'],'FontSize',14);
