...
%%
lats = 1:55;
intlev = 27; dlev = 700; % 700m
dweit = depthData(2:intlev)-depthData(1:intlev-1); % depth weight
Tsubraw = permute(nansum(Tempa(:,lats,1,:)+Tempa(:,lats,2:intlev,:).*(permute(dweit,[3 2 1])),3)/dlev,[1 2 4 3]); 
...
% print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\Tem0-700m_trend_',num2str(startyr),'_',num2str(endyr),'.png'],'-dpng','-r300')
%%
close all
ftsz = 14;
...
print(Fig,['D:\figures\IAP\Yearly\20230911_IPO_SouthernOcean_46_61S\Tem0-700m_trend_colorbar.png'],'-dpng','-r300')
%% raw non-detrended
lats = [29:44];
...
legend('boxoff')
title('Original')
% print(Fig,['D:\figures\IAP\Yearly\20230911_IPO_SouthernOcean_46_61S\time_non-detrended.png'],'-dpng','-r300')
%% raw detrended
lats = [29:44];
...
legend('boxoff')
title('Detrended')
% print(Fig,['D:\figures\IAP\Yearly\20230911_IPO_SouthernOcean_46_61S\time_detrended.png'],'-dpng','-r300')
%%
lats = 1:55;
...
print(Fig,['D:\figures\IAP\Yearly\20230321_IPO_SouthernOcean_48_63S\Com_raw_1979_1993.png'],'-dpng','-r300')
...
m_line(160,[-61:1:-46],'linewidth',2,'color','k');
set_colorbar([0.83 0.08 0.03 0.88],'K',5,ftsz,[-ticks:ticks/5:ticks],[-ticks:ticks/5:ticks])
print(Fig,['D:\figures\IAP\Yearly\20230321_IPO_SouthernOcean_48_63S\Com_raw_1994_2012.png'],'-dpng','-r300')
%% load Temperature
%-----------------------------------------------------------------------------
% clc,clear,close all;
...
datadir='D:\data\ERA5_1x1\10m_wind\'; %指定批量数据所在的文件夹
...
%%  load slp
datadir='D:\data\ERA5_1x1\sea_level_pressure\'; %指定批量数据所在的文件夹
filelist=dir([datadir,'*.nc']); %指定批量数据的类型
ncdisp([datadir,filelist(1).name]);
filetemp=[datadir,filelist(1).name]; %查看你要读取的文件的编号。filelist（1）.name在window下为第一个标号的数据
ncid=netcdf.open(filetemp,'NC_NOWRITE');
...
%% load hadley sst
filename = 'D:\data\hadley\HadISST_sst.nc'; % 1870-2021
...
%% load ERSSTv5 sst
filename = 'D:\data\ERSSTv5\sst.mnmean.nc'; % 1854-2021
ncid=netcdf.open(filename,'NOWRITE');
ncdisp(filename);
sste = ncread(filename,'sst');
lonER = ncread(filename,'lon');
latER = ncread(filename,'lat');
%
sste(find(sste == -1.8)) = nan;
sste = permute(mean(reshape(sste,[180 89 12 2040/12]),3),[1 2 4 3]); % yearly 
sstea = double(sste(:,:,67:167) - mean(sste(:,:,128:157),3)); % 1920-2020; remove climatology from 1981 to 2010
sstear = permute(sstea,[3 1 2]);
dT = 1; cf = 1/8;
clear sstead ssteadf
for i = 1:180
    for j = 1:89
    sstead(:,i,j) = detrend(sstear(:,i,j)); % linear detrend
    ssteadf(:,i,j) = lanczosfilter(sstead(:,i,j),dT,cf,[],'low'); % 8 year filtered
    end
end
ssteadf = permute(ssteadf,[2 3 1]);
ssteadf(:,:,1:4) = []; ssteadf(:,:,end-3:end) = [];
% detrend 
Temphadi4 = permute(sstea,[3 1 2]);
clear Tsdhadi1
...
% IPO index
[ts1_zsh ts1h] = areamean(sstea,71:108,22:32,latER); % 25N-45N,140E-145W
[ts2_zsh ts2h] = areamean(sstea,86:136,40:50,latER); % 10S-10N,170E-90W
[ts3_zsh ts3h] = areamean(sstea,76:101,53:70,latER); % 50S-15S,150E-160W
TPI1h = ts2h-(ts1h+ts3h)/2; % unfiltered IPO index
dT = 1; % interval
cf = 1/8;
TPI_filtered8_ER = lanczosfilter(TPI1h,dT,cf,[],'low'); % 8 year filtered IPO index

%% load surface heat data
datadir='D:\data\ERA5_1x1\heat\'; %指定批量数据所在的文件夹
filelist=dir([datadir,'*.nc']); %指定批量数据的类型
ncdisp([datadir,filelist(1).name]);
filetemp=[datadir,filelist(1).name]; %查看你要读取的文件的编号。filelist（1）.name在window下为第一个标号的数据
k=length(filelist);
...
% print(Fig,['D:\figures\IAP\Yearly\nhflx_ERA5.png'],'-dpng','-r300')
%% load z200
datadir='D:\data\ERA5_1x1\Geopotential\'; %指定批量数据所在的文件夹
filelist=dir([datadir,'*.nc']); %指定批量数据的类型
ncdisp([datadir,filelist(1).name]);
filetemp=[datadir,filelist(1).name]; %查看你要读取的文件的编号。filelist（1）.name在window下为第一个标号的数据
ncid=netcdf.open(filetemp,'NC_NOWRITE');
...
lats = 1:90;
intlev = 27; dlev = 700; % 700m
% intlev = 12; dlev = 100; % 100m
dweit = depthData(2:intlev)-depthData(1:intlev-1); % depth weight
Tsub_long = permute(nansum(cat(3,Tempadlf_long(:,lats,1,:),Tempadlf_long(:,lats,2:intlev,:).*(permute(dweit,[3 2 1]))),3)/dlev,[1 2 4 3]); % 0-700m, 1940-2020
Tsub = permute(nansum(cat(3,Tempadlf(:,lats,1,:),Tempadlf(:,lats,2:intlev,:).*(permute(dweit,[3 2 1]))),3)/dlev,[1 2 4 3]); % 0-700m
Tsub100 = permute(nansum(cat(3,Tempadlf(:,lats,1,:),Tempadlf(:,lats,2:12,:).*(permute((depthData(2:12)-depthData(1:11)),[3 2 1]))),3)/100,[1 2 4 3]); % 0-100m
...
% print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\Tem0-700m_EOFmap_0_90S_zscore.png'],'-dpng','-r300')
%% DI index
lats = [29:44];
...
%%  time series TPI & Nino3.4
TPIfz_long = zscore(TPIf_long);
load('D:\data\NOAA\indices\Nino3.4_HadIsst1_anomaly_1981-2010_baseline.txt');
Nino34 = mean(Nino3_4_HadIsst1_anomaly_1981_2010_baseline(51:151,2:13),2); % 1920-2020
Nino34z = zscore(Nino34);
Nino34f = lanczosfilter(Nino34,1,1/8,[],'low');
close all;
Fig = figure('position',[100 100 800 400])
mapp = find(Nino34z > 0); mapn = find(Nino34z < 0);
bar(mapp,Nino34z(mapp),'FaceColor',[0.87,0.68,0.68],'EdgeColor',[0.87,0.68,0.68],'barwidth',1)
hold on
bar(mapn,Nino34z(mapn),'FaceColor',[0.6,0.75,0.84],'EdgeColor',[0.6,0.75,0.84],'barwidth',1)
plot([1:97],TPIfz_long(1:97),'color',[.8 .05 .05],'LineWidth',2)
% plot([1:101],zscore(Nino34),'color',[.8 .05 .05],'LineWidth',0.5)
plot(zeros(1,101),'k','LineWidth',1)
plot([97:101],TPIfz_long(97:101),'--','color',[.8 .05 .05],'LineWidth',2)
set(gca,'XLim',[1,101]);
...
% print(Fig,['D:\figures\IAP\Yearly\20240130_IPO_SO\TimeSeries_IPO_Nino3.4.png'],'-dpng','-r300')
%%  time series DI & TPI & AMO
load('MatFile/TPI_filtered13_hadley.mat','TPI_filtered13_hadley');
TPIf_13 = TPI_filtered13_hadley(51:151); % 1920-2020
TPIf_13z = zscore(TPIf_13);
mapp = find(TPIf_13z > 0); mapn = find(TPIf_13z < 0);
TPIfz_long = zscore(TPIf_long);
AMOfz_long = zscore(AMOf_long);
close all;
Fig = figure('position',[100 100 800 400])
bar(mapp,TPIf_13z(mapp),'FaceColor',[0.87,0.68,0.68],'EdgeColor',[0.87,0.68,0.68],'barwidth',1)
hold on
bar(mapn,TPIf_13z(mapn),'FaceColor',[0.87,0.68,0.68],'EdgeColor',[0.87,0.68,0.68],'barwidth',1)
p1 = plot([5+20:77+20],DI,'-','color','k','LineWidth',2)
p2 = plot([1:97],TPIfz_long(1:97),'color',[.8 .05 .05],'LineWidth',2)
p3 = plot([1:97],AMOfz_long(1:97),'color',[.13 .27 .86],'LineWidth',2)
plot(zeros(1,101),'k','LineWidth',1)
plot([1+20:4+20],DI_long(1:4),'--','color','k','LineWidth',2)
plot([78+20:81+20],DI_long(78:81),'--','color','k','LineWidth',2)
plot([97:101],TPIfz_long(97:101),'--','color',[.8 .05 .05],'LineWidth',2)
plot([97:101],AMOfz_long(97:101),'--','color',[.13 .27 .86],'LineWidth',2)
...
set(gca,'YTick',[-3:1:3]);
xlabel('YEAR','FontSize',12),ylabel('Normalized index','FontSize',14);
legend([p1,p2,p3],'PmAI','IPO','AMO','Location','north','Orientation','horizontal')
legend('boxoff');
print(Fig,['D:\figures\IAP\Yearly\20240130_IPO_SO\TimeSeries_DI_IPO_AMO.png'],'-dpng','-r300')
%% time series -- 
...
legend('boxoff');
print(Fig,['D:\figures\IAP\Yearly\20230911_IPO_SouthernOcean_46_61S\TimeSeries_',pngname,'.png'],'-dpng','-r300')
%% DI&IPO scatter plot
close all;
num_pc1p = find(DI>0);
num_pc1n = find(DI<0);
...
saveas(new_fig_handle,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\scatter_DI_IPO.png'],'png')
...
lg = legend([p1,p2,p3],{str2,'POGA$-$LEM','LEM'},'Location','northeast','position',[0.72,0.73,0.19,0.16],'Orientation','vertical','interpreter','latex')
legend('boxoff');
print(Fig,['D:\figures\CESM\Yearly\PAC-pacemaker\20231024_IPO_SouthernOcean_46_61S\ensmean\Times_IAP_Pacemaker_LE_',str1,'.png'],'-dpng','-r300')
...
xlabel('YEAR','FontSize',12),ylabel([str1, ' (K)'],'FontSize',14);
legend([p1,p2,p3],{str2,'POGA$-$LEM','LEM'},'Location','northeast','position',[0.72,0.73,0.19,0.16],'Orientation','vertical','interpreter','latex')
legend('boxoff');
print(Fig,['D:\figures\CESM\Yearly\PAC-pacemaker\20231024_IPO_SouthernOcean_46_61S\ensmean\Times_IAP_Pacemaker_LE_',str1,'.png'],'-dpng','-r300')
%% IPO time series ERSSTv5 with CESM PAC pacemaker
itIAP = TPIf_long; 
itER = TPI_filtered8_ER; 
itpac = TPIpac; str1 = 'IPO'; str2 = 'Hadisst'; yval = 1;
itensm = TPIensm; 
close all;
Fig = figure('position',[100 100 800 400])
maxval = max(itpac,[],2);
...
p1 = plot([1:97],itIAP(1:end-4),'k-','LineWidth',3);
p2 = plot([5:90],itensm(5:90),'-','color',[.8 .05 .05],'LineWidth',3);
p3 = plot([1:97],itER(1:end-4),'-','color','b','LineWidth',1.5);
plot([97:101],itIAP(end-4:end),'k:','LineWidth',3)
plot([1:5],itensm(1:5),':','color',[.8 .05 .05],'LineWidth',3)
plot([90:94],itensm(end-4:end),':','color',[.8 .05 .05],'LineWidth',3)
plot([97:101],itER(end-4:end),'--','color','b','LineWidth',1.5)
% plot([90:94],itLE(end-4:end),'--','color','b','LineWidth',1.5)
plot(44*ones(20,1),[-1:2/19:1],'color',[0.18,0.60,0.49],'LineWidth',1.5); % 1963, Agung volcanic
plot(63*ones(20,1),[-1:2/19:1],'color',[0.18,0.60,0.49],'LineWidth',1.5); % 1982, El Chichon volcanic
plot(72*ones(20,1),[-1:2/19:1],'color',[0.18,0.60,0.49],'LineWidth',1.5); % 1991, Pinatubo volcanic
...
set(gca,'YTick',[-yval:yval/2:yval]);
xlabel('YEAR','FontSize',12),ylabel([str1, ' (K)'],'FontSize',14);
legend([p1,p2,p3],{str2,'POGA$-$LEM','ERSSTv5'},'Location','northeast','position',[0.72,0.73,0.19,0.16],'Orientation','vertical','interpreter','latex')
legend('boxoff');
print(Fig,['D:\figures\IAP\Yearly\20240130_IPO_SO\Times_ERSST_Pacemaker_LE_',str1,'.png'],'-dpng','-r300')
...
% print(Fig,['D:\figures\IAP\Yearly\20221121_IPO_SouthernOcean\Tem0-700m_EOFmap_0_90S_Pac.png'],'-dpng','-r300')
%% PC1/DI/IPO .reg. meridional mean 
lats = [29:44]; names = '46S-61S'; index = PC1z; pngname = 'PC1';
...
print(Fig,['D:\figures\IAP\Yearly\20240130_IPO_SO\',pngname,'.reg.Tem0-700m.png'],'-dpng','-r300')
%% IPO reg 0-700m Tem
var = permute(Tsub100(:,1:55,:),[3 1 2]); % 100m
...
m_line(160,[-61:1:-46],'linewidth',2,'color','k'); 
set_colorbar([0.83 0.08 0.03 0.88],'K',4.5,ftsz,[-ticks:ticks/5:ticks],[-ticks:ticks/5:ticks])
print(Fig,['D:\figures\IAP\Yearly\20240130_IPO_SO\',pngname,'_reg_Tem0-100m.png'],'-dpng','-r300')
...
testdots(h3,[.4 .4 .4],lonDataE,latDataE)
m_line([0:1:360],-46,'linewidth',2,'color','k'); 
m_line([0:1:360],-61,'linewidth',2,'color','k'); 
m_line(-60,[-61:1:-46],'linewidth',2,'color','k'); 
m_line(160,[-61:1:-46],'linewidth',2,'color','k'); 
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.windstress&slp.png'],'-dpng','-r300')
%% curlz & wind
close all;
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.windstress&curlz.png'],'-dpng','-r300')
%% taux
close all;
ftsz = 12; ticks1 = 5;
Fig = figure('position',[10 50 550 400]);
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.Taux.png'],'-dpng','-r300')
%% tauy
ftsz = 12; ticks1 = 5;
close all;
Fig = figure('position',[10 50 550 400]);
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.Tauy.png'],'-dpng','-r300')
...
% print(Fig,['D:\figures\IAP\Yearly\20230206_IPO_SouthernOcean\PC1.reg.meridionalmean.Vwindstress.50_60S.png'],'-dpng','-r300')
...
legend('boxoff')
saveas(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\legend1.png'])
...
m_line(-60,[-61:1:-46],'linewidth',2,'color','k'); 
m_line(160,[-61:1:-46],'linewidth',2,'color','k'); 
% print(Fig,['D:\figures\IAP\Yearly\20221121_IPO_SouthernOcean\PC1_corr_UV.png'],'-dpng','-r300')
%% PC1/DI/IPO .reg.SST
index = DI; pngname = 'DI';
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.sst.png'],'-dpng','-r300')
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.nhflx.png'],'-dpng','-r300')
%% PC1 1std composite 
...
% print(Fig,['D:\figures\IAP\Yearly\20230206_IPO_SouthernOcean\PC1p_0.5std.com.Tem700m.png'],'-dpng','-r300')
clear map2
for s = 1:length(evs_PC1n)
    map2(:,:,s) = mean(Tsub(:,:,evs_PC1n{s}),3);
end
[Fig] = com_map(mean(map2,3),0.3,lonData,latData(1:90))
% print(Fig,['D:\figures\IAP\Yearly\20230206_IPO_SouthernOcean\PC1n_0.5std.com.Tem700m.png'],'-dpng','-r300')
...
set(gca,'YTickLabel',[700:-100:0],'FontSize',14);
ylabel('Depth (m)')
print(Fig,['D:\figures\IAP\Yearly\20230206_IPO_SouthernOcean\PC1n_1std.com.meridionalmeanT.png'],'-dpng','-r300')
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pocname,'.regwith.',varname,'.scatter.png'],'-dpng','-r300')
...
% print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\MOHT_climotology.png'],'-dpng','-r300')
...
% print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.zonalmean_MOHT_Ekman.png'],'-dpng','-r300')
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.reg.zonalmean_MOHT&Taux.png'],'-dpng','-r300')
%%
Fig = figure('position',[100 100 800 400]);
p1 = plot(latData,par7/10^15,'r-','linewidth',1.5);
hold on
...
legend('boxoff')
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\legend2.png'],'-dpng','-r300')
...
legend('boxoff')
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\legend3.png'],'-dpng','-r300')
...
print(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\IPO.reg.Z200za.png'],'-dpng','-r300')
...
% filename = 'D:\data\NOAA\20CRv3\20thC_ReanV3\Monthlies\miscSI-MO\prmsl.mon.mean.nc'; % 1836-2015
    ncid=netcdf.open(filename,'NOWRITE');
    ncdisp(filename);
...
saveas(Fig,['D:\figures\IAP\Yearly\20230606_IPO_SouthernOcean_46_61S\',pngname,'.png'])
end
