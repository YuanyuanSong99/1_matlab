...
legend('boxoff')
% print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/TimeSeries_Tem_original_',depthstr,'m_35S_90S.png'],'-dpng','-r300')
...
% print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/Tem',depthstr,'m_trend_',num2str(startyr),'_',num2str(endyr),'.png'],'-dpng','-r300')
%% meridional mean
lats = 35:55; % 90S-35S
latstr = '35S_55S';
...
ylabel('Depth (m)')
% print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/meridionalmean_',latstr,'_trend_',yrstr,'.png'],'-dpng','-r300')
%% 150 longitude
close all;
ftsz = 12;
ticks = 0.1;
...
set(gca,'YTickLabel',[2000:-400:0],'FontSize',14);
ylabel('Depth (m)')
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/meridionalmean_',latstr,'_trend_',yrstr,'_150E.png'],'-dpng','-r300')
...
ro = 1025 % kg m-3
Temp1 = Tempo(:,:,:,1:528);
Temp2 = reshape(Temp1,[720 360 50 12 528/12]);
Temp = squeeze(mean(Temp2,4));
%------------------- ------ 0-2000m -----------------------------------------
% depthstr = '0-2000m';
% dweit = depthData(2:41)-depthData(1:40); % depth weight
...
OHCsub = cp*ro*permute(nansum(cat(3,Temp(:,:,1,:)*5,Temp(:,:,2:26,:).*(permute(dweit,[3 2 1]))),3),[1 2 4 3]); % J/m2

OHCw = OHCsub/365/24/60/60; % W/m2
%% OHC trend
startyr = 1980;
endyr = 2020;
var = permute(OHCw(:,:,startyr-1979:endyr-1979),[3 1 2]);
x = [1:size(var,1)]';
clear trd 
for i = 1:size(var,2);
    for j = 1:size(var,3);
...
print(Fig,['/Users/yysong/Desktop/figures/OCCA2/Yearly/20240503_SO_warming/OHC',depthstr,'_trend_',num2str(startyr),'_',num2str(endyr),'.png'],'-dpng','-r300')
%% Global plot
close all;
ftsz = 12; ticks = 15;
...
testdots0(h0,[.4 .4 .4],lonData,latData);
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/OHC',depthstr,'_trend_',num2str(startyr),'_',num2str(endyr),'.2.png'],'-dpng','-r300')
...
spacV = cp*ro*squeeze(nansum(nansum(nansum(Vzyxsub_r_0(1:140,lats,1:intlev),1),2),3));
spac0V = spac_0./spacV;
sia_0 = cp*ro*squeeze(nansum(nansum(nansum(Tzyxsub_r_0(141:360,lats,1:intlev,:),1),2),3));
siaV = cp*ro*squeeze(nansum(nansum(nansum(Vzyxsub_r_0(141:360,lats,1:intlev),1),2),3));
sia0V = sia_0./siaV;
sdiff = spac0V-sia0V; 
max(spac0V,[],'all')

zpac = cp*ro*squeeze(nansum(nansum(Tzyxsub_r_0(1:140,1:90,1:intlev,:),1),3)); % zonal sum
pacS = cp*ro*squeeze(nansum(nansum(Vzyxsub_r_0(1:140,1:90,1:intlev),1),3));
zpacS = zpac./pacS';
zatl = cp*ro*squeeze(nansum(nansum(Tzyxsub_r_0(141:230,1:90,1:intlev,:),1),3)); % zonal sum
atlS = cp*ro*squeeze(nansum(nansum(Vzyxsub_r_0(141:230,1:90,1:intlev),1),3));
zatlS = zatl./atlS';
zio = cp*ro*squeeze(nansum(nansum(Tzyxsub_r_0(231:360,1:90,1:intlev,:),1),3)); % zonal sum
ioS = cp*ro*squeeze(nansum(nansum(Vzyxsub_r_0(231:360,1:90,1:intlev),1),3)); 
zioS = zio./ioS';
%% OHC/m3  time series
close all;
Fig = figure('position',[700 100 800 400]);
...
legend('boxoff')
% print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/TimeSeries_OHC_Wm3_',depthstr,'_35S_90S.png'],'-dpng','-r300')
...
legend('boxoff')
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/TimeSeries_OHC_PW_',depthstr,'m_35S_90S_',bsln,'.png'],'-dpng','-r300')
%% zonal OHC
lonstr = 150;
startyr = 1960;
endyr = 2020;
...
% datadir='/Volumes/Togo/data/ERA5_1x1/10m_wind/'; %指定批量数据所在的文件夹
...
datadir='/Volumes/Togo/data/ERA5_1x1/windstress/'; %指定批量数据所在的文件夹
filelist=dir([datadir,'*.nc']); %指定批量数据的类型
ncdisp([datadir,filelist(1).name]);
filetemp=[datadir,filelist(1).name]; %查看你要读取的文件的编号。filelist（1）.name在window下为第一个标号的数据
ncid=netcdf.open(filetemp,'NC_NOWRITE');
...
%%  load slp
datadir='/Volumes/Togo/data/ERA5_1x1/sea_level_pressure/'; %指定批量数据所在的文件夹
filelist=dir([datadir,'*.nc']); %指定批量数据的类型
ncdisp([datadir,filelist(1).name]);
filetemp=[datadir,filelist(1).name]; %查看你要读取的文件的编号。filelist（1）.name在window下为第一个标号的数据
ncid=netcdf.open(filetemp,'NC_NOWRITE');
...
%% load sst and sic
filename = '/Volumes/Togo/data/hadley/HadISST_sst.nc'; % 1870-2021
...
filename = '/Volumes/Togo/data/hadley/HadISST_ice.nc'; % 1870-2021
ncid=netcdf.open(filename,'NOWRITE');
ncdisp(filename);
...
datadir='/Volumes/Togo/data/ERA5_1x1/heat/'; %指定批量数据所在的文件夹
filelist=dir([datadir,'*.nc']); %指定批量数据的类型
ncdisp([datadir,filelist(1).name]);
filetemp=[datadir,filelist(1).name]; %查看你要读取的文件的编号。filelist（1）.name在window下为第一个标号的数据
k=length(filelist);
...
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/',varname,'_climo_1960_2020_Polar_ERA5.png'],'-dpng','-r300')
%% trend
startyr = 1960;
endyr = 2020;
var = permute(nhflx(:,:,startyr-1939:endyr-1939),[3 1 2]);
trd = trend_cal_3D(var);
h0 = trendtest(var,0.05); % t test trend
h0(find(isnan(Temp(:,:,1,1)) == 1)) = nan;
max(trd,[],'all'), min(trd,[],'all')
%%
% units = '10^-^8 Pa m^-^1'; name = 'CURLZ_Er'; 
...
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/',name,'_trend_',num2str(startyr),'_',num2str(endyr),'_3.png'],'-dpng','-r300')
%% area mean Time series (anomaly)
% lats = [126:146]; latstr = '35-55S';
% lats = [146:161]; latstr = '55-70S';

lats = [126:156]; latstr = '35-65S'; % zonal wind
...
% [sat_0] = areamean(var_r_0(:,:,21:81),141:231,lats,latDataE); 
...
% tt11 = 27; tt12 = 21;
% tt21 = 42; tt22 = 21;
% ymax = 20; ymin = -15; yint = 5;
% tt01 = 14; tt02 = 16;
% tt11 = 27; tt12 = 16;
% tt21 = 42; tt22 = 16;

% var_r_0 = cat(1,curlz_Er(150:360,:,:),curlz_Er(1:149,:,:)); 
...
% ln2 = sat_0*10^8;  max(ln2)
% ln3 = sio_0*10^8; min(ln3)
% % ymax = 12; ymin = 2; yint = 2;
% % tt01 = 14; tt02 = 10.8;
% % tt11 = 27; tt12 = 10.8;
% % tt21 = 42; tt22 = 10.8;
% ymax = -3; ymin = -21; yint = 3;
% tt01 = 14; tt02 = -5.2;
% tt11 = 27; tt12 = -5.2;
% tt21 = 42; tt22 = -5.2;

var_r_0 = cat(1,uwst(150:360,:,:),uwst(1:149,:,:)); 
...
ln2 = sat_0*100; min(ln2)
ln3 = sio_0*100; min(ln3)
ymax = 21; ymin = 6; yint = 3;
tt01 = 14; tt02 = 19;
tt11 = 27; tt12 = 19;
tt21 = 42; tt22 = 19;
% ymax = 18; ymin = 0; yint = 3;
% tt01 = 14; tt02 = 15.5;
% tt11 = 27; tt12 = 15.5;
% tt21 = 42; tt22 = 15.5;
% ymax = 16; ymin = 8; yint = 2;
% tt01 = 14; tt02 = 15;
% tt11 = 27; tt12 = 15;
% tt21 = 42; tt22 = 15;
...
legend('boxoff')
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/',str2,'_TimeSeries_',latstr,'_',bsln,'.png'],'-dpng','-r300')
%% zonal mean 1
% name = 'NHFLX_zonalm';
% var_raw = nhflx;
% name = 'CURLZ_zonalm';
...
varpac = permute(mean(var_r(1:140,:,:,:),1,'omitmissing'),[2 3 4 1]); % Pac unit:W
varatl = permute(mean(var_r(141:231,:,:,:),1,'omitmissing'),[2 3 4 1]); % Atl unit:W
...
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/',name,'_trend_',num2str(startyr),'_',num2str(endyr),'_Pac_150E.png'],'-dpng','-r300')
%
Fig = plotzm_nhflx(climap2,trd2,'Atlantic',latDataE)
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/',name,'_trend_',num2str(startyr),'_',num2str(endyr),'_Atl_150E.png'],'-dpng','-r300')

Fig = plotzm_nhflx(climap3,trd3,'Indian Ocean',latDataE)
print(Fig,['/Users/yysong/Desktop/figures/IAP/Yearly/20240503_SO_warming/',name,'_trend_',num2str(startyr),'_',num2str(endyr),'_IO_150E.png'],'-dpng','-r300')
%% curlz plot
...
%% geostrophic MOHT
datadir='/Volumes/Togo/data/IAP/geostrophic current/IAP_Ocean_geostrophic_current_0_2000m/'; %指定批量数据所在的文件夹
filelist=dir([datadir,'*.nc']); %指定批量数据的类型
ncdisp([datadir,filelist(1).name]);
filetemp=[datadir,filelist(1).name]; % 查看你要读取的文件的编号。filelist（1）.name在window下为第一个标号的数据
ncid=netcdf.open(filetemp,'NC_NOWRITE'); 
...
% ro*cp*OHC*Vy
% Vy = My/ro = -taux/f/ro 
datadir='/Volumes/Togo/data/IAP/temperature/Yearly/'; %指定批量数据所在的文件夹
...
% filename = '/Volumes/Togo/data/NOAA/20CRv3/20thC_ReanV3/Monthlies/miscSI-MO/prmsl.mon.mean.nc'; % 1836-2015
    ncid=netcdf.open(filename,'NOWRITE');
    ncdisp(filename);
