% era5 forcing Data 
% Note: Results have some errors when calculate OHC using 'dx' and MOHT
% using 'londist'. If you wants to make the results more comparable, you
% must use the same distance of two longitudes.
clc,clear;
...
ylabel('Depth (m)')
print(Fig,['/Users/yysong/Desktop/figures/LICOM/Yearly/20240705_SO_warming/CTR_Tem.meridionalmean_',latstr,'_trend_',yrstr,'_150E.png'],'-dpng','-r300')
%% OHC
cp = 4096 % J (kg oC)
ro = 1025 % kg m-3
%------------------------- 0-2000m -----------------------------------------
...
OHCsub = cp*ro*permute(nansum(cat(3,Tem1(:,:,1,:)*5,Tem1(:,:,2:21,:).*(permute(dweit,[3 2 1])),Tem700*(700-depthData_r(21))),3),[1 2 4 3]); % J/m2
...
print(Fig,['/Users/yysong/Desktop/figures/LICOM/Yearly/20240705_SO_warming/CTR_OHC',depthstr,'_trend_',num2str(startyr),'_',num2str(endyr),'.3.png'],'-dpng','-r300')
...
Tempzy = Tempz*1000; % 111 km / latitude
VVzy = VVz*111*1000;
...
dy = 111*1000;
OHC_0 = cp*ro*dy*dx.*cat(3,Tem1(:,:,1,:)*5,Tem1(:,:,2:21,:).*(permute(dweit,[3 2 1])),Tem700*(700-depthData_r(21))); % J/m2
%
...
elon = 1000*sw_dist([45,45],[1,2],'km');
map_p = OHC_zm_p/(60*60*24*365)/(140*elon*700);
map_ia = OHC_zm_ia/(60*60*24*365)/(210*elon*700);

%%
%close all
figure(3)
plot([1:55],map_p(1:55),'b')
hold on
plot([1:55],map_ia(1:55),'r')
