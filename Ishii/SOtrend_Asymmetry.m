...
%% Tem
Tempa = double(Temps - nanmean(Temps(:,:,:,27:56),4)); % remove climatology from 1981-2010 
sst_Ishii = permute(Tempa(:,:,1,:),[1 2 4 3]);
lats = 1:60;
%------------------------- 0-2000m -----------------------------------------
% depthstr = '0-2000';
% dweit = depthData(2:26)-depthData(1:25); % depth weight
% Tsubraw = permute(nansum(cat(3,Temps(:,lats,1,:),Temps(:,lats,2:26,:).*(permute(dweit,[3 2 1]))),3)/2000,[1 2 4 3]); 
%------------------------- 0-700m -----------------------------------------
depthstr = '0-700';
dweit = depthData(2:16)-depthData(1:15); % depth weight
Tsubraw = permute(nansum(cat(3,Temps(:,lats,1,:),Temps(:,lats,2:16,:).*(permute(dweit,[3 2 1]))),3)/700,[1 2 4 3]); 
...
p1 = bar([1:67],sdiff,'FaceColor',[0.7,0.7,0.7],'EdgeColor',[0.7,0.7,0.7],'BarWidth',1);
hold on
set(gca,'YLim',[ymin,ymax],'YTick',[ymin:yint:ymax],'YColor','r');
ylabel('Difference  (K)','FontSize',14);
yb = polyfit([1:67],sdiff,1);
plot([1:67],polyval(yb,[1:67]),'--','Color',[0.3,0.3,0.3],'linewidth',2)
...
yb = polyfit([1:67],spac_0,1);
plot([1:67],polyval(yb,[1:67]),'--','Color',[.04 .35 .56],'linewidth',2)
...
legend('boxoff')
% print(Fig,['/Volumes/Togo4T/figures/Ishii/Yearly/20231104_SO_warming/TimeSeries_Tem_original_',depthstr,'m_35S_90S.png'],'-dpng','-r300')
%% meridional mean
lats = 35:55; % 55S-35S
latstr = '35S_55S';
...
endyr = 2020;
yrstr = '1960-2020';
var = permute(Tempa_mm(:,:,startyr-1954:endyr-1954),[3 1 2]);
x = [1:size(var,1)]';
clear trd
for i = 1:size(var,2);
    for j = 1:size(var,3);
...
ylabel('Depth (m)')
print(Fig,['/Volumes/Togo4T/figures/Ishii/Yearly/20240101_SO_warming/Tem_meridionalmean_',latstr,'_trend_',yrstr,'.png'],'-dpng','-r300')
%% 150 longitude
close all;
ftsz = 12;
ticks = 0.1;
...
set(gca,'YTickLabel',[2000:-400:0],'FontSize',14);
ylabel('Depth (m)')
print(Fig,['/Users/yysong/Desktop/figures/Ishii/Yearly/20240508_SO_warming/Tem_meridionalmean_',latstr,'_trend_',yrstr,'_150E.png'],'-dpng','-r300')
%% OHC
cp = 4096 % J (kg oC)
ro = 1025 % kg m-3
%------------------------- 0-2000m -----------------------------------------
...
depthstr = '0-700m';
dweit = depthData(2:16)-depthData(1:15); % depth weight
OHCsub = cp*ro*permute(nansum(cat(3,Temps(:,:,1,:),Temps(:,:,2:16,:).*(permute(dweit,[3 2 1]))),3),[1 2 4 3]); % J/m2
...
map = nanmean(OHCw(:,:,yrs2-1954:yrs2-1954+yrspan-1),3)-nanmean(OHCw(:,:,yrs1-1954:yrs1-1954+yrspan-1),3);
max(map,[],'all')
min(map,[],'all')
% SO asymmetry
close all;
...
% print(Fig,['/Volumes/Togo4Tfigures/Ishii/Yearly/20231107_SO_warming/Delta_OHC_W_raw',depthstr,'_',num2str(yrs1),'_',num2str(yrs2),'_',num2str(yrspan),'yrs.png'],'-dpng','-r300')
%% OHC trend
startyr = 1980;
endyr = 2020;
var = permute(OHCw(:,:,startyr-1954:endyr-1954),[3 1 2]);
x = [1:size(var,1)]';
clear trd 
for i = 1:size(var,2);
    for j = 1:size(var,3);
...
print(Fig,['/Users/yysong/Desktop/figures/Ishii/Yearly/20240508_SO_warming/OHC',depthstr,'_trend_',num2str(startyr),'_',num2str(endyr),'.3.png'],'-dpng','-r300')
%% Global plot
close all;
ftsz = 12; ticks = 15;
...
testdots0(h0,[.4 .4 .4],lonData,latData);
print(Fig,['/Volumes/Togo4T/figures/Ishii/Yearly/20240101_SO_warming/OHC',depthstr,'_trend_',num2str(startyr),'_',num2str(endyr),'.2.png'],'-dpng','-r300')
%% original OHC time series
lats = [1:55]; % 90S-35S
levs = [1:16]; % 700 m
VV = ones(length(lonData),length(latData),length(depthData)); % calculate volume
Temps = double(Temps);
Tempz = cat(3,Temps(:,:,1,:),Temps(:,:,2:26,:).*(permute(dweit,[3 2 1])));
VVz = cat(3,VV(:,:,1),VV(:,:,2:26).*(permute(dweit,[3 2 1])));
...
spacV = cp*ro*squeeze(nansum(nansum(nansum(Vzyxsub_r_0(1:121,lats,levs),1),2),3));
spac0V = spac_0./spacV;
sia_0 = cp*ro*squeeze(nansum(nansum(nansum(Tzyxsub_r_0(122:360,lats,levs,:),1),2),3));
siaV = cp*ro*squeeze(nansum(nansum(nansum(Vzyxsub_r_0(122:360,lats,levs),1),2),3));
sia0V = sia_0./siaV;
sdiff = spac0V-sia0V;
max(spac0V,[],'all')
%% OHC/m3  time series
...
saveas(Fig,['/Volumes/Togo4T/figures/Ishii/Yearly/20230606_IPO_SouthernOcean_46_61S/',pngname,'.png'])
end
